...
function homeFallback(language: "ru" | "en") {
  ...
  return `<div class="seo-static-home">
    ...
    <main>
      ...
      <section class="seo-static-case" id="work"><h2>${ru ? "Избранные работы: продукты, 3D и Telegram" : "Selected work: products, 3D and Telegram"}</h2><p>${ru ? "Nebo Bistro — запущенный бот и Mini App. DROP — рабочий интерактивный 3D-концепт магазина. AI Support / 30 — клиентский Telegram MVP на этапе проектирования." : "Nebo Bistro is a live bot and Mini App. DROP is a working interactive 3D commerce concept. AI Support / 30 is a client Telegram MVP currently in design."}</p><div class="seo-static-actions"><a href="${prefix}/projects/nebo-bistro">${ru ? "Разобрать Nebo Bistro" : "Explore Nebo Bistro"} →</a><a href="https://drop-air-force-1.quad-thatch-5fteamfa.chatgpt.site">${ru ? "Открыть DROP" : "Open DROP"} →</a></div></section>
